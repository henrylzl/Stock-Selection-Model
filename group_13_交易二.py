...
def on_data(context):
    ...
    if current_date in context.fre_date:  
        ...
        # 选择特征和目标变量。
        X = df[['log_NC', 'LEV', 'NI_p', 'NI_n', 'g', 'log_RD', 'SWNLMY', 'SWMT', 'SWJCHG', 'SWGT', 'SWYSJS', 'SWDZ', 'SWJYDQ', 'SWSPYL', 'SWFZFS',
                        'SWQGZZ', 'SWYYSW', 'SWGYSY', 'SWJTYS', 'SWFDC', 'SWSMLS', 'SWSHFW', 'SWZH', 'SWJZCL',
                        'SWJZZS', 'SWDLSB', 'SWGFJG', 'SWXXFW1', 'SWCM', 'SWTX', 'SWYH', 'SWFYJR', 'SWQC', 'SWJXSB']]
        Y = df[['log_mcap']]
        # 填充特征和目标变量中的缺失值。
...
def handle_stock_orders(context, mp_stock):  # 股票调仓函数
    ...
    for i in range(len(mp_stock)):
        if mp_stock.loc[i, 'target_percent'] >= 0:
            # 调仓到目标持仓量
            # account_idx---账户索引号，target_idx---标的资产索引号，target_volume---目标委托量，
            # side---头寸方向（0不明，1多头，2空头），order_type---委托类型（0不明，1开仓，2平仓，3今平仓），price---委托单价格
...
def get_recent_quarter_data_all(order_book_ids, date, field_bs=(), field_cf=(), field_ins=()): # 获取最近一个季度的财务数据
    """
...
def mad_winsorize_series(se, sigma_n): # 极值处理
    """
...
def winsorize_series(se, limits=0.025, inclusive=False): # 极值处理
    """
    对给定的序列进行极值处理（缩尾处理）
